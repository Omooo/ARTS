...
[S.O.L.I.D Principles: The Kotlin Way](https://medium.com/the-android-caf%C3%A9/solid-principles-the-kotlin-way-ff717c0d60da)

使用 Kotlin 的方式来演示 SOLID 设计原则。

Single Responsibility 单一职责：

单一职责说的是一个类只能有一个职责，换句话说，如果修改它的原因不止一个，那么就不是一个单一职责的类。

Open-Close 开闭原则：

开闭原则说的是，一个类应该对扩展开放，对修改关闭。文章中举例的是对一个图形面积的计算很不错，可以看一下。

Liskov Substitution 里氏替换原则：

里氏替换原则说的是，用子类替换父类不需要改动其他代码也能保证功能正常。这个原则是指导我们，在设计子类时，应该是扩展父类功能，还不是修改父类已有实现。

Interface Segregation 接口隔离原则：

接口隔离原则说的是，接口要尽可能小，不能大而全。如果接口中有些方法不是每个实现类都必须要实现的话，那就可能违背了接口隔离原则。文字举的动物会飞的例子就很形象。

Dependency Inversion 依赖倒置原则：

依赖倒置原则说的是，高层模块不依赖于低层模块，它们共同依赖同一个抽象。抽象不要依赖具体实现，具体实现依赖于抽象。这个原则是指导我们，在设计低层模块时，低层模块提供的接口要足够的抽象、通用，而不要随意改动。
